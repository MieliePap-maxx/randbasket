...
assert.match(appSource, /catalogue-price-unavailable/, "catalogue products without a live price must remain visible");
...
assert.match(appSource, /locationSession\.write\(location\)/, "granted coordinates must be shared for the current browsing session");
assert.match(appSource, /locationSession\.read\(\)/, "session coordinates must be restored after navigation or refresh");
assert.match(html, /\.\.\/location-session\.js\?v=1/, "the app must load the shared session location bridge");
assert.match(landingSource, /heroLocation\.query\(location\)/, "the landing comparison must send available coordinates");
assert.match(landingSource, /pageshow/, "the landing comparison must refresh after returning from the app");
...
assert.match(appSource, /service-worker\.js\?v=38/, "the page must request the latest service worker");
assert.doesNotMatch(html, /maxResultsInput/, "catalogue pagination must not be presented as a total product limit");
assert.match(
  appSource,
  /filter\(\(store\) => state\.settings\.stores\?\.\[store\.storeId\] !== false\)/,
  "catalogue results must respect disabled retailers",
);
assert.match(
  appSource,
  /state\.specials\.filter\(/,
  "catalogue specials must respect disabled retailers",
);
